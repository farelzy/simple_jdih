...
interface Opd { id: number; kode: string; nama_resmi: string; nama_singkat: string }
  ...
  async function kirim() {
    ...
    try {
      const r = await panggilApi<{ sukses: boolean; nomor: string }>('/api/pengajuan/kirim', {
        method: 'POST',
        body: JSON.stringify({
          draf, jenis_peraturan: jenis, opd: namaOpd, judul,
    ...
    <div className="wadah-sempit">
      ...
      {langkah === 2 && (
        <section className="kartu">
          ...
          {opd.length > 0 ? (
            <label>
              Nama OPD Pemohon
              <select value={namaOpd} onChange={(e) => setNamaOpd(e.target.value)}>
                <option value="">&mdash; pilih &mdash;</option>
                {opd.map((o) => <option key={o.id} value={o.nama_resmi}>{o.nama_resmi}</option>)}
              </select>
            </label>
          ) : (
            <label>
              Nama OPD Pemohon
              <input value={namaOpd} onChange={(e) => setNamaOpd(e.target.value)} placeholder="BPKAD" />
              <span className="petunjuk">
                Daftar OPD belum diisi Bagian Hukum, jadi diketik dulu.
              </span>
            </label>
          )}
      ...
      {langkah === 4 && (
        <section className="kartu">
          ...
          <p><strong>OPD:</strong> {namaOpd || '(belum diisi)'}</p>
